...
public class JwtAuthenticationFilter extends OncePerRequestFilter{
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            // Request Header에 있는 Bearer Token 을 가져옴
            String token = parstBearToken(request);
    ...
    private String parstBearToken(HttpServletRequest request) {
        // Request Header의 Authorization 필드의 Value를 가져옴
        String autorizationValue = request.getHeader("Authorization");
        
        // Authorization Value가 Bearer로 시작되는지
        boolean isBearer = autorizationValue.startsWith("Bearer ");
        if (!isBearer) return null;

        // Authorization Value에 문자가 포함되어 있는지
        boolean hasTokenValue = StringUtils.hasText(autorizationValue);
        if (!hasTokenValue) return null;
        
        // "Bearer " 다음에 오는 문자열()을 추출
        String token = autorizationValue.substring(7);
        return token;
    }
