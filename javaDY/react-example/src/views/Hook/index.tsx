...
//? React Hooks must be called in React function component or a custom React Hook function
//? 위 에러를  반환.
...
export default function Hook() {
  ...
  const onClickHandler = () => {
    ...
    //! 실제 값을 변경하고 다시 set 매서드로 적용시켜 준다.
  ...
  const onPlusHandler = () => {
    ...
    setNum(num + 1);
    //^ 그렇기 때문에 이래와 같이 변경 후에 변경 값으로 작업을 하려고 해도
    //^ 원래 저장되어있는 값으로 작업이 진행된다.
    alert(num);
    //! 이런 문제를 해결하는 방법
  ...
  //^ useEffect에 scope 한 state를 useEffect 내에서
