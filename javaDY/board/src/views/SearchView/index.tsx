...
export default function SearchView() {
    ...
    const{ onPageHandler, viewList, pageNumber, boardList, COUNT, setBoardList } = usePagingHook();
    // const COUNT = 5;
    ...
    useEffect(() => {
        ...
        //? 해당 문자열에서 검색할 문자열이 존재한다면 true, 아니면 false
  ...
  return (
    <Box sx={{p: '40px 120px', backgroundColor: 'rgba(0, 0, 0, 0.05)'}}>
        <Box sx={{fontSize:'24px', fontWeight: 500}}>
            ...
            <Typography component='span' sx={{opacity:0.7}}>에 대한 검색결과입니다.</Typography>
        ...
        <Box sx={{pt : '20px', pb: '80px'}}>
            <Grid container spacing={3}>
                <Grid item sm={12} md={8}>
                    <Stack spacing={2}>
                        {viewList.length === 0 ? (<Box sx={{ height: '460px', display: 'flex', justifyContent: 'center', alignItems: 'center'}}><Typography sx={{fontSize: '24px', fontWeight: 500, color: 'rgba(0, 0, 0, 0.4)'}}>검색결과를 찾을 수 없습니다.</Typography></Box>) : viewList.map((boardItem) => (<BoardListItem item={boardItem}/>))}
                    </Stack>
                </Grid>
