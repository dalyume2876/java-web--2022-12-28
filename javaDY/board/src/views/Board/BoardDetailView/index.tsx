...
export default function BoardDetailView() {
    ...
    useEffect(() => {
        ...
        //? BOARD_LIST 에서 boardNumer에 해당하는 board를 가져옴
        ...
        const owner = user !== null && user.nickname === board.writeNickname
        setMenuFlag(owner);
        setBoard(board);
    }, [])
        ...
        <Box>
            <Box>
                ...
                <Box sx={{ mt:'20px', display: 'flex', justifyContent: 'space-between'}}>
                    <Box sx={{display: 'flex', alignItems: 'center'}}>
                        ...
                        <Typography sx={{mr: '8px', fontSize:'16px', fontWeight: 500}}>{board?.writeNickname}</Typography>
            ...
            <Box sx={{display: 'flex', mt:'20px'}}>
                <Box sx={{display:'flex', mr:'20px'}}>
                    { likeStatus ? 
                    (<FavoriteOutlinedIcon sx={{height: '24px', width: '24pz', mr:'6px', opacity: 0.7, color: '#ff0000'}} onClick={() => setLikeStatus(!likeStatus)} />) :
                    (<FavoriteBorderIcon sx={{height: '24px', width: '24pz', mr:'6px', opacity: 0.7}} onClick={() => setLikeStatus(!likeStatus)} />) 
                    }
